    ...
    protected ModelAndView doGet(HttpRequest httpRequest, HttpResponse httpResponse) {
        try {
            if (httpRequest.getCookies().getOrDefault("logined", "false").equals("true")) {
                ...
                for (Memo memo : memos) {
                    block += String.format(
                            "<tr>\n<th scope=\"row\">%s</th> <td>%s</td> <td>%s</td></tr>\n",
                            memo.getDate().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")), memo.getName(), memo.getContent());
                }
                ...
                String view = "<!DOCTYPE html>\n" +
                        ...
                        "       <a href=\"/create\" class=\"btn btn-primary pull-right\" role=\"button\">질문하기</a>\n" +
                        "</div>"+
                        "\n" +
                        "<!-- script references -->\n" +
