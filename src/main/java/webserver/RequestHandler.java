    ...
    public void run() {
        ...
        try (InputStream in = connection.getInputStream(); OutputStream out = connection.getOutputStream()) {
            ...
            if(path.startsWith("/user/create")){
                ...
                User user = new User(params.get("userId"),params.get("password"),params.get("name"), URLDecoder.decode(params.get("email")));
                log.debug("User : {}", user);
                ...
                List<User> users = DataBase.findUsers();
            ...
            }else if(path.startsWith("/qna/show")){
                log.debug("qna/show");
                StringBuilder sb = new StringBuilder();
                List<Post> posts = DataBase.findPosts();
                ...
                sb.append("<!DOCTYPE html>\n" +
                        ...
                        "              <thead>\n" +
                        "                <tr>\n" +
                        "                    <th>날짜</th> <th>작성자</th> <th>내용</th><th></th>\n" +
                        "                </tr>\n" +
                        "              </thead>\n" +
                        "              <tbody>\n");
                ...
                try{
                    ...
                    bw.close();
                    DataOutputStream dos = new DataOutputStream(out);
                    byte[] body = Files.readAllBytes(new File("./webapp/qna/show.html").toPath());
                    response200Header(dos, body.length);
                    responseBody(dos, body);
                }catch(Exception e){
                    log.debug(e.getMessage());
